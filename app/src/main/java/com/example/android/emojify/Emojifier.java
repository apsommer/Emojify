...
class Emojifier {
    ...
    private static void whichEmoji(Face face) {
        ...
        // TODO (3): Create threshold constants for a person smiling, and an eye being open by taking pictures of yourself and your friends and noting the logs.
        ...
        boolean isSmiling = face.getIsSmilingProbability() > threshold;
        boolean isLeftEyeOpen = face.getIsLeftEyeOpenProbability() > threshold;
        boolean isRightEyeOpen = face.getIsRightEyeOpenProbability() > threshold;

        // TODO (5): Create an if/else system that selects the appropriate emoji based on the above booleans and log the result.
        Emoji emoji;
        if (isSmiling) {
            if (isLeftEyeOpen) {
                if (isRightEyeOpen) emoji = Emoji.SMILING;
                else emoji = Emoji.RIGHT_WINK;
            } else {
                if (isRightEyeOpen) emoji = Emoji.LEFT_WINK;
                else emoji = Emoji.CLOSED_EYE_SMILING;
            }
        } else {
            if (isLeftEyeOpen) {
                if (isRightEyeOpen) emoji = Emoji.FROWNING;
                else emoji = Emoji.RIGHT_WINK_FROWNING;
            } else {
                if (isRightEyeOpen) emoji = Emoji.LEFT_WINK_FROWNING;
                else emoji = Emoji.CLOSED_EYE_FROWNING;
            }
        }
        Log.e("~~", emoji.toString());
    }
